...
from src.api.geoname_specifier.city_name_formatter import (CityNameFormatter, CityNameFormatterBasic,
                                                           CityNameFormatterWithCommas, )
from src.api.geoname_specifier.geoname_sorter import (GeoNameSorter, GeoNameSorterBasic,
...
class Business:
    ...
    def get_geonames_specified(city_name: str,
        ...
        city_name_formatted: str = city_name_formatter.get_city_name_formatted(city_name)

        geoname_query: BaseQuery = GeoName.query \
            .filter(GeoName.feature_class == 'P') \
            .filter(GeoName.alternatenames.contains(city_name_formatted))
